...
MongoMapper.database = 'sgs_dansguardian'
...
def parse_logfile(logfile)
  ...
  puts "Calculation size of #{logfile}..."

  total = %x{wc -l #{logfile}}.split.first.to_i

  puts "Working on #{logfile}, size #{total}..."
  ...
  begin
    ...
    while ( line = file.gets )
      ...
      if a[5] == ''
        ...
        # size = 16
        if a[6] =~ /Exception_Site_Match/ or a[6] =~ /Exception_URL_Match/ or a[6] =~ /Exception_Client_IP_Match/
...
if ARGV.size > 0 then
  logfile = ARGV.each do |logfile|
    puts "$0: Working on #{logfile}..."
    parse_logfile(logfile)
  end
else
  puts "Usage $0 dansguardian_file(s)"
end
